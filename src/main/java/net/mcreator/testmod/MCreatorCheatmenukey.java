...
public class MCreatorCheatmenukey extends Elementstestmod.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public MCreatorCheatmenukey(Elementstestmod instance) {
		super(instance, 24);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.cheatmenukey", GLFW.GLFW_KEY_F4, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (!(Minecraft.getInstance().currentScreen instanceof ChatScreen)) {
			if (event.getKey() == keys.getKey().getKeyCode() && event.getAction() == GLFW.GLFW_PRESS) {
				testmod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
